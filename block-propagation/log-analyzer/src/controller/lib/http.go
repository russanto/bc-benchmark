...
// This file is part of the go-ethereum library but modified by russanto to fit his needs.
//
...
// DialHTTPWithClientAndAuth is the DialHTTPWithClient version with auth header
func DialHTTPWithClientAndAuth(endpoint string, client *http.Client, username string, password string) (*Client, error) {
	req, err := http.NewRequest(http.MethodPost, endpoint, nil)
	if err != nil {
		return nil, err
	}

	authValuesEncoded := base64.StdEncoding.EncodeToString([]byte(username + ":" + password))

	req.Header.Set("Content-Type", contentType)
	req.Header.Set("Accept", contentType)
	req.Header.Set("Authorization", "Basic "+authValuesEncoded)
